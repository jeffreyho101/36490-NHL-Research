...
for link in range(0, len(teams_links)):
    ...
    driver.get(url)
    jsonfile = "https://stats.tsn.ca/HGET/urn:tsn:nhl:team:" + teams_links[link] + "/roster?type=json"
    driver.get(jsonfile)
    data = json.dumps(jsonfile)

    print data

    # for row in driver.find_elements(By.CLASS_NAME, 'stats-table-scrollable'):
