...
nhl.data.clean1$DaysMissed = as.integer(end.date - start.date) / 86400
...
write.table(as.matrix(nhl.data.clean1), "Clean-Data/NHL_Clean_V13.txt", sep="\t")

```

```{r message=FALSE, warning=FALSE}
teamnames = na.omit(unique(nhl.data.clean1$TeamName))
...
```{r}
library(msm)
# need to duplicate data points in order to classify date into injury status (ie. injury start date -> injured = 1, injury end date -> injured = 0)
nhl.data.double = nhl.data.clean1[rep(1:nrow(nhl.data.clean1),each=2),] 
...
Q = matrix(c(0,1,1,0), nrow=2,ncol=2, dimnames=list(from=1:2,to=1:2))
# basic bidirectional model of injury mapping
cav.msm = msm(InjuryIndicator ~ YearsSince2000, subject=FirstLast, data = nhl.data.double, 
               qmatrix = Q, deathexact = NULL, exacttimes=TRUE)

# MSM with previously injured as a covariate (0/1 boolean)
cav.injured.msm = msm(InjuryIndicator ~ YearsSince2000, subject=FirstLast, data = nhl.data.double, 
